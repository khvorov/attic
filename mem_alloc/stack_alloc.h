...
private:
    typename std::aligned_storage<sizeof(T) * N, std::alignment_of<T>::value>::type buf_;
    pointer ptr_;

public:
    stack_alloc() noexcept : ptr_((pointer)&buf_) {}
    stack_alloc(const stack_alloc&) noexcept : ptr_((pointer)&buf_) {}
    template <class U>
    stack_alloc(const stack_alloc<U, N>&) noexcept : ptr_((pointer)&buf_) {}
    ...
    constexpr size_type max_size() noexcept { return size_type(~0) / sizeof(T); }
